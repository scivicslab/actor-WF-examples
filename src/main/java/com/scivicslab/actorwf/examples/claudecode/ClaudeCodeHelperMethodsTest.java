...
/**
 * Test application demonstrating helper methods for common interaction patterns.
 * <p>
 * This test showcases the convenience methods provided by ClaudeCodeSession
 ...
 * @version 1.0
 */
public class ClaudeCodeHelperMethodsTest {
    ...
    public static void main(String[] args) {
        ActorSystem system = new ActorSystem.Builder("claude-helpers-system").build();

        String sessionName = "claude-helpers-" + System.currentTimeMillis();
        ...
        try {
            ...
            String question = "I'm building a web application. Suggest 3 backend frameworks " +
            ...
            System.out.println("Selecting: option 2 with additional question");

            ClaudeCodeOutput selectionResponse = sessionRef.ask(s -> {
                try {
                    return s.selectChoice(2, "What are the main advantages and disadvantages?");
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }).get();
            ...
            System.out.println("\n=== Test 3: Send follow-up question ===");
            String followUp = "Which companies use this framework in production?";
            ...
            // Test 4: Interrupt and redirect conversation
            System.out.println("\n=== Test 4: Interrupt and redirect ===");
            System.out.println("Starting a long request...");

            // Start a potentially long operation in background
            Thread longTask = new Thread(() -> {
                try {
                    sessionRef.ask(s -> {
                        try {
                            System.out.println("Sending: Write a comprehensive guide to microservices architecture...");
                            return s.sendPrompt("Write a comprehensive guide to microservices architecture " +
                                              "including all design patterns and best practices.");
                        } catch (Exception e) {
                    ...
                    System.out.println("Background task interrupted: " + e.getMessage());
                }
            });

            longTask.start();

            // Simulate user changing mind after 3 seconds
            Thread.sleep(3000);
            System.out.println("\n*** User changes mind - interrupting and redirecting ***");

            ClaudeCodeOutput redirected = sessionRef.ask(s -> {
                try {
                    return s.interruptAndSendNew(
                        "Actually, forget that. Just tell me in one sentence what microservices are."
                    );
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            ...
            System.out.println("\n--- Redirected response ---");
            printOutput(redirected);

            longTask.join(2000);

            // Test 5: Fallback on timeout
            System.out.println("\n=== Test 5: Fallback behavior ===");
            System.out.println("Trying request with 10-second timeout and fallback...");

            ClaudeCodeOutput fallbackResponse = sessionRef.ask(s -> {
                try {
                    return s.sendWithFallback(
                        "Generate a detailed comparison table of 10 programming languages " +
                        "with code examples for each",
                        10000,  // 10 second timeout
                        "Just list 3 programming languages briefly"
                    );
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            ...
            System.out.println("\n--- Response (initial or fallback) ---");
            printOutput(fallbackResponse);

            // Test 6: Verify context retention across all operations
            System.out.println("\n=== Test 6: Verify context retention ===");
            String contextCheck = "What was the web application framework I chose earlier?";
