 ...
 *
 * <p>This class wraps a CodexSession object and allows it to be invoked
 * by the workflow interpreter using string-based action names. It provides
 * a bridge between the workflow system and the Codex session implementation,
 * enabling workflow-driven Codex interaction.</p>
 *
 * @author Scivics Lab
 * @version 1.0
 */
public class CodexSessionIIAR extends IIActorRef<CodexSession> {

    private ClaudeCodeOutput lastOutput;

    /**
     * Constructs a new CodexSessionIIAR with the specified actor name and session instance.
     *
     * @param actorName the name of the actor
     * @param session the Codex session instance
     * @param system the actor system managing this actor
     */
    public CodexSessionIIAR(String actorName, CodexSession session, IIActorSystem system) {
        super(actorName, session, system);
    }

    /**
     * Invokes a Codex session action by name.
     *
    ...
    /**
     * Gets the last output from Codex.
     *
